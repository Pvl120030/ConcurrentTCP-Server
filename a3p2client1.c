...
while(fgets(buffer,255,stdin)) 
{
  ...
  if(strcmp(buffer,"\n") != 0)
  {
    while(1)
   {
    ...
    //writing to log 
    out = fopen("/home/012/p/pv/pvl120030/A3/a3p2client1.log", "a");
    if(out==NULL){ printf("FILE OPEN ERROR");}
    fprintf(out,"Message from server:\n %s", serverMsg);
    fputs("\n", out);
    fclose(out);
    
    printf("Client sleep for three seconds\n");
    sleep(3);
    printf("Client waking up... \n");
    printf("Current local time and date: %02d:%02d:%02d %02d/%02d/%04d \n", tm.tm_hour, tm.tm_min, tm.tm_sec, tm.tm_mon+1, tm.tm_mday, tm.tm_year+1900);sleep(3);
    }
